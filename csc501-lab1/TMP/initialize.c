...
int fhead,ftail,shead,stail,ttail,thead,lhead,ltail;
...
LOCAL int sysinit()
{
	...
	*( (int *)pptr->pbase ) = MAGIC;
	...
	pptr->pargs = 0;
	pptr->pprio = 0;
	pptr->flag = FLAGNOT;  /*0 specifies that the process should not run.*/
	pptr->exhaust = EXHAUSTNOT;
	currpid = NULLPROC;
	...
	ftail   = 1 + (fhead = newqueue());
	stail = 1 + (shead = newqueue());
	ttail = 1 + (thead = newqueue());
	ltail = 1 + (lhead = newqueue());
